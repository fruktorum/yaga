...
module YAGA
  class Population(T, V)
    ...
    def initialize(@total_bots = TOTAL_BOTS, @selection_bots = SELECTION_BOTS, @mutation_percent = MUTATION_PERCENT, @crossover_enabled = true, @random = Random.new, &block : Int32 -> Bot(T, V))
    ...
    def initialize(@total_bots = TOTAL_BOTS, @selection_bots = SELECTION_BOTS, @mutation_percent = MUTATION_PERCENT, @crossover_enabled = true, @random = Random.new)
      raise ArgumentError.new("Total bots amount should be greater than or equal to 16, got: #{@total_bots}") if @total_bots < 16
      raise ArgumentError.new("Selection bots amount should be at most half of total bots, got: #{@selection_bots}") if @selection_bots * 2 > @total_bots
